...
- **observed fact** — `REMOTE_SHA256SUMS` preserves that CPFS layout and is
  not the repository-local verifier for its duplicate `analysis/` paths.
  Repository-local integrity is provided by `BUNDLE_SHA256SUMS` and the
  80-file `RAW_SHA256SUMS`.
