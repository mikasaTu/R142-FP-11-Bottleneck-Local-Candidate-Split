...
"$PYTHON" - "$CHECKPOINT" "$CHECKPOINT_TREE_SHA" <<'PY' > runtime/checkpoint_validation.json
...
for path in sorted(root.rglob("*")):
    ...
    aggregate.update(f"{digest}  ./{path.relative_to(root).as_posix()}\n".encode())
    count += 1
observed = aggregate.hexdigest()
if observed != expected: raise SystemExit(f"checkpoint tree SHA mismatch: {observed} != {expected}")
