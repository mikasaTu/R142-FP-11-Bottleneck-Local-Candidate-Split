...
| runtime-independent-20260903 | not created | prepared, no PAI submission | A runtime is frozen to `/mnt/cpfs/zbl-cpfs-new/USERS/leon/code/r142-stage-s-a-runtime-20260903` @ `a1e280b64bd0eb522b036e05007f57373f403020`; external payload path remains `/mnt/cpfs/zbl-cpfs-new/USERS/leon/code/r142-stage-s-pai-20260902/stage_s_robotwin_a_pai.sh`. Payload hash is bound by the canonical config and revalidated before CreateJob. |
...
| authority-root-validation-20260903 | not eligible | fail-closed before submission | The canonical controller registry readback was not clean (`HEAD=1e70bd145c04f3c21f64b691232cefdc67d51846`, tracked and untracked changes present), so no clean-validation claim is made. The exact canonical validator invocation against this final config returned exit code 2: `REFUSED: unknown resource alias: idle-a800-robot-stage-s-graphics-8gpu`. The required alias was not substituted, no PAI job was submitted, and the canonical `FROZEN_PROTOCOL.json`/`ACCEPTED_A_ASSET_PREFLIGHT.json` authorities are absent, so the launcher remains fail-closed. |
