...
| `r142-stage-s-a-assets-20260902-r8` | pending submission | prepared | Uses a new pinned Python 3.11 tools environment inside the Stage-S cache, bootstrapped with conda and invoked through `python -m pip`. Completion still requires terminal PAI success plus `COMPLETED_ASSET_PREFLIGHT.json` and `SHA256SUMS` verification. |
