...
## Execution ledger (controller/CPFS readback)

These attempts are operational evidence only; neither is an accepted B
calibration result.
...
| `r142-stage-s-b-calibration-20260903-r4` | `dlcy9156hxh7b2q3` | terminal `Stopped`; no scientific work | The derived artifact path passed, then the source gate incorrectly required `.git` to be a directory. Pinned OpenPI and LIBERO are valid submodule worktrees whose `.git` entries are files; both pass `git rev-parse --is-inside-work-tree` and exact commit readback. The job was stopped before `FIRST_WORK`; logs/readback/SHA are preserved under the corresponding `failures/r4` directory. The gate now asks Git directly and keeps the same exact commit/clean-tree checks. |
