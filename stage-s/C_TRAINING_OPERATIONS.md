...
DEPLOY_DIR=/mnt/cpfs/zbl-cpfs-new/USERS/leon/code/r142-stage-s-pai-20260902
DEPLOY_PAYLOAD=$DEPLOY_DIR/stage_s_c_undertrained_pai.sh
DEPLOY_CONFIG=$DEPLOY_DIR/stage_s_c_undertrained.json
...
contract separately requires exactly
`NVIDIA_DRIVER_CAPABILITIES=compute,utility,graphics`; the registry injects
only `PAI_CANARY_RUN_ID`. The launcher obtains the source commit and payload
digest from its hard-coded pins and external companion config, so no custom
pod environment is needed.
...
`pai-job-registry`; it binds the external command file, robot-idle
alias/id/quota, UID/GID, CPFS write paths, pinned Python, sequential stage
payload, and application autoresume. Deploy both
`$DEPLOY_PAYLOAD` and `$DEPLOY_CONFIG` under the canonical code root before
validation. The launcher checks the hard-coded runtime Git HEAD and
cleanliness, the QPILOTS parent commit, the OpenPI commit, the companion
config SHA, and both the external and executed-payload SHA; missing or
mismatched values fail closed. The payload itself is non-interactive and
never changes `HOME`.
